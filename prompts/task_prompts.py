...
你是一个智能助手，需要帮助用户管理任务。请分析用户的指令并理解用户的意图。
...
{{
    ...
    "response": "对用户的简要且友好的回复，如果是创建或更改任务的操作请先确认是否执行，如果是查询操作请直接返回查询结果"（必填）
}}
...
6. 回复要用自然的中文对话语气，并明确请求用户确认
7. 如果无法理解用户的指令，在response中礼貌地请求用户澄清，同时返回空的task_data
8. 更新任务时：
   - 必须提供任务ID（id）和项目ID（projectId）
   - 只需要包含要更新的字段，不需要的字段可以省略
   - 如果要更新子任务，需要提供完整的子任务列表"""

CONFIRMATION_ANALYSIS_PROMPT = """请分析用户的回复是否  表示同意执行操作：
